        ...
        <!-- 商品属性 -->
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(choose,index) in spuSaleAttrList" :key="choose.id">
                <dt class="title">选择{{choose.saleAttrName}}</dt>
                <dd changepirce="0" :class="{active:attr.isChecked==1}" 
                v-for="(attr,index2) in choose.spuSaleAttrValueList" :key="attr.id"
                @click="changeActive(choose.spuSaleAttrValueList,attr.id)"
                >
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model.number="skuNum" @input="inspect"/>
...
export default {
  ...
  methods: {
    //vue中的数据是响应式的，你可以直接以参数的形式传递过来然后修改，影响原数据。这一切vue都帮我们做好了
    changeActive(list,id){
      list.forEach(element => {
        element.isChecked='0'
        if(element.id===id) element.isChecked='1'
      });
    },
    //对用户输入购买数量的检查
    inspect(){
      if(this.skuNum<0) this.skuNum=1
      this.skuNum=Math.floor(this.skuNum)
    },
    //发起添加购物车的请求并跳转路由
    async addGoods(){
      //仓库中发请求的结果是一个promise，我们接收这个结果并结局不同的结果做出不同响应
      try{
            await this.$store.dispatch('addOrUpdataGoods',{skuId:this.$route.params.skuId, skuNum:this.skuNum.toString()})
            //如若请求成功，我们将对addCart组件今天跳转并发送参数。
            //但由于我们要用信息比较多，所以我们只通过query传一部分，剩下的通过临时会话存储传递过去
