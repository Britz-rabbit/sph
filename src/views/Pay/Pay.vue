...
export default {
  ...
  methods: {
    ...
    async generateQR(text) {
      try {
        //toDataURL内传入payInfo.codeUrl，其返回结果是图片的src
    ...
    //点击立即支付后弹出支付二维码
    async payNow() {
      let qr = await this.generateQR(this.payInfo.codeUrl);
      //此处标签的判断即为严格：首先img必须紧挨着前闭合标签，/必须紧挨着后闭合标签，其次/前必须有一个空格
      this.$alert(`<img src=${qr} />`, "请使用微信支付", {
        ...
        beforeClose: (action, instance, done) => {
          if (action == "cancel") {
            ...
            //该地方的逻辑是：利用定时器不停的向服务器发请求确认状态。如果定时器没有，则开启；支付有结果后需要关闭定时器
            //带块代码为自动跳转
          if (!this.timer) {
              this.timer = setInterval(async () => {
                ...
                if (res.code == 200) {
                  ...
                  clearInterval(this.timer);
                  this.timer = null;
                  //将支付查询的状态码保存，当用户点击‘已完成支付’后进行核对跳转
